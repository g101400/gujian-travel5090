...
copy /Y "..\\app.ico" app.ico >nul 2>&1
...
echo [3/3] 链接(light)...
"%WIX%bin\light.exe" -ext WixUIExtension -out "古建景点打卡_Setup.msi" build_msi.wixobj files.wixobj
if exist "古建景点打卡_Setup.msi" ( echo ✅ 产出：古建景点打卡_Setup.msi ) else ( echo ❌ 生成失败 )
endlocal
pause

...
(function () {
  ...
  function openBookmarkChooser(list) {
    ...
    var rows = list.map(function (bm, i) {
      ...
      return '<div class="bm-row"><span style="flex:1;font-size:13px">' + ((bm.name || ("窗口 " + (i + 1))).replace(/[<>&"]/g, function (c) { return ({ "<": "&lt;", ">": "&gt;", "&": "&amp;", "\"": "&quot;" })[c]; })) + ' <span style="color:#888">（' + ts + '）</span></span>' +
  ...
  function buildMenu() {
    ...
    // v3.7.7：智能化内核（模糊检索 / 提示词生成 / AI记忆·Hermes / 存疑反向查询 / 强制联网）
  ...
  function openPlatformCompare() {
    ...
    var rows = [
      ["智能化内核：智能模糊检索（错字 / 简写 / 别名）", "✅", "✅", "✅", "✅", "菜单「智能化内核 → 🔎 智能模糊检索」，带相似度排序"],
      ["智能化内核：提示词生成器（6 套模板）", "✅", "✅", "✅", "✅", "巡检/维修/参数核对/汇报/隐患/培训，可复制或直接提问"],
      ["智能化内核：AI 记忆 · Hermes 沉淀", "✅", "✅", "✅", "✅", "对话记忆查看清空 + 经验沉淀，后续问答自动引用"],
      ["智能化内核：存疑反向查询（来源 + 数值冲突）", "✅", "✅", "✅", "✅", "反查出处并列出待核实差异，可一键联网核实"],
      ["智能化内核：强制联网开关", "✅", "✅", "✅", "✅", "开启后强制走在线模型，断网明确报错"],
      ["菜单收藏 / 隐藏 API 补齐（getFavMenus 等）", "✅", "✅", "✅", "✅", "根治打开菜单报 getFavMenus is not defined"],
      ["GitHub 升级（检测新版 + 四平台安装包）", "✅", "✅", "✅", "✅", "设置 → GitHub 升级，公开仓库免登录"],
  ...
  function openChangelog() {
    ...
    var html = '<div class="changelog-ver"><span class="cv">3.7.9</span><span class="cd">2026-09-10</span></div>' +
      ...
      "<li>新增（知识库智能化内核 · 菜单「智能化内核」）：①智能模糊检索（错字/简写/别名，带相似度排序，可就地提问）；②提示词生成器（6 套业务模板，可复制或一键提问）；③AI 记忆 · Hermes（对话记忆查看/清空 + 经验沉淀，后续问答自动引用）；④存疑反向查询（反查出处 + 数值冲突待核实清单，可联网核实）；⑤强制联网开关（开启后强制走在线模型，断网明确报错，不再静默降级）。</li>" +
      "<li>回归保持：打开菜单报 getFavMenus is not defined 根治、隐藏/收藏二次确认、安卓导入 ovkmz 自动定位与缩放、GitHub 升级；四端功能对照单与版本变更同步至 3.7.7。</li>" +
      '</ul>' +
  ...
  _deferBoot(function () {
    ...
    if (window.CtxMenu) try { window.CtxMenu.init({ map: map, records: function () { return HERITAGE; }, textFields: ["intro", "features"], getCoordinates: getCoordinates, nearbySearch: nearbySearch }); _bootMark("ctxMenu"); } catch (e) {}
    _bootMark("bootDone");
  });
...
var upPwaCfg = { single: "https://g101400.github.io/gujian-travel5090/" };;
function ghChannel() {
